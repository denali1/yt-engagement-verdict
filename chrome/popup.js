/**
 * chrome/popup.js
 * YT Engagement Verdict — Chrome MV3 Popup Script
 *
 * Identical to popup.js but uses chrome.* instead of browser.*
 */
...
(function () {
  ...
  chrome.storage.local.get("telemetry_opted_in").then(result => {
  ...
  toggle.addEventListener("change", () => {
    ...
    chrome.storage.local.set({ telemetry_opted_in: optedIn });
    toggleLabel.textContent = optedIn ? "On" : "Off";
    toggleLabel.style.color = optedIn ? "#4caf50" : "#555";
  });
  ...
  chrome.tabs.query({ active: true, currentWindow: true }).then(tabs => {
    ...
    if (isYouTubeVideo) {
      chrome.tabs.sendMessage(tab.id, { type: "GET_SELECTOR_VERSION" })
        .then(res => {
    ...
    if (refreshBtn) {
      refreshBtn.addEventListener("click", () => {
        ...
        chrome.tabs.sendMessage(tab.id, { type: "FORCE_SELECTOR_REFRESH" })
          .then(res => {
    ...
    chrome.tabs.sendMessage(tab.id, { type: "GET_VERDICT" }).then(response => {
